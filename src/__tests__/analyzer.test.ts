import { NpmDependencyAnalyzer, AnalysisResult, MultiPackageAnalysisResult } from '../index';
...
describe('NpmDependencyAnalyzer', () => {
    ...
    describe('Single Package Analysis', () => {
        it('should analyze express package successfully', async () => {
            const result: AnalysisResult = await analyzer.analyze('express', '4.18.2');
        ...
        it('should throw error for invalid package', async () => {
            await expect(analyzer.analyze('invalid-package-name-123456', '1.0.0')).rejects.toThrow();
        });

        it('should throw error for invalid version', async () => {
            await expect(analyzer.analyze('express', 'invalid-version')).rejects.toThrow();
        });
    ...
    describe('Multiple Package Analysis', () => {
        it('should analyze multiple packages successfully', async () => {
            ...
            const result: MultiPackageAnalysisResult = await analyzer.analyze(packages);
            expect(result).toBeDefined();
    ...
    describe('Registry Configuration', () => {
        it('should use default registry when not specified', async () => {
            ...
            const result: AnalysisResult = await defaultAnalyzer.analyze('react', '18.2.0');
            expect(result).toBeDefined();
            NpmDependencyAnalyzer.printDependencyTree(result.dependencyTree);
            expect(consoleSpy).toHaveBeenCalled();
        }, 30000);
        ...
        it('should handle registry timeout', async () => {
            ...
            await expect(timeoutAnalyzer.analyze('express', '4.18.2')).rejects.toThrow();
        });
    });
});
