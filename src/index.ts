...
export class NpmDependencyAnalyzer {
    ...
    private async getPackageInfo(name: string, version: string): Promise<PackageInfo> {
        ...
        try {
            const response = await fetch(`${this.config.registry}/${name}/${version}`, {
            ...
            if (!data.name || !data.version) {
                throw new PackageNotFoundError(name, version, new Error('Invalid package data format'));
            }
            ...
            throw new PackageNotFoundError(name, version, error instanceof Error ? error : new Error(String(error)));
        }
    }
    ...
    private async buildDependencyTree(
        ...
        const currentPath = parentPath ? `${parentPath} > ${name}@${packageInfo.version}` : `${name}@${packageInfo.version}`;
        ...
        const dependencyPromises = Object.entries(packageInfo.dependencies ?? {}).map(
            async ([depName, depVersion]) => {
                const depNode = await this.buildDependencyTree(depName, depVersion, flatDeps, currentPath);
        ...
        for (const [name, dep] of hoistedTree.root) {
            ...
            if (dep.peerDependencies.size > 0) {
                ...
                for (const [peerName, peerVersion] of dep.peerDependencies) {
                    ...
                    const isSatisfied = rootPeer && semver.satisfies(rootPeer.version, peerVersion);
                    ...
                    if (dep.peerDependencies.size > 0) {
                        ...
                        for (const [peerName, peerVersion] of dep.peerDependencies) {
                            ...
                            if (rootPeer && semver.satisfies(rootPeer.version, peerVersion)) {
                                isSatisfied = true;
                            } else if (parentPeer && semver.satisfies(parentPeer.version, peerVersion)) {
                                isSatisfied = true;
                            }
    ...
    private convertToHoistedTree(node: DependencyNode, parentPath: string = ''): HoistedTree {
        ...
        const convertToHoistedDep = (node: DependencyNode, parent?: string): HoistedDependency => ({
            name: node.name,
            version: node.version,
            dependencies: new Map(
                Array.from(node.dependencies.entries()).map(([name, dep]) => [name, dep.version])
            ),
        ...
        const hasVersionConflict = (dep1: HoistedDependency, dep2: DependencyNode): boolean => {
        ...
        const canBeHoisted = (dep: DependencyNode, targetPath: string = ''): boolean => {
            ...
            for (const [peerName, peerVersion] of dep.peerDependencies) {
                ...
                if (hoistedPeer && !semver.satisfies(hoistedPeer.version, peerVersion)) {
                    return false;
                }
            }
        ...
        const processNode = (
            ...
            if (!hoistedTree.root.has(node.name)) {
                ...
                if (canBeHoisted(node)) {
                    ...
                    hoistedTree.nested.get(parentPath)!.set(
                        node.name,
                        convertToHoistedDep(node, parentPath)
                    );
                }
            } else {
    ...
    async analyzeSingle(packageName: string, version: string): Promise<AnalysisResult> {
        const flatDeps = new Map<string, FlatDependency>();
        const dependencyTree = await this.buildDependencyTree(packageName, version, flatDeps);
    ...
    async analyzeMultiple(packages: PackageRequest[]): Promise<MultiPackageAnalysisResult> {
     ...
     */
    public async analyze(packageName: string, version: string): Promise<AnalysisResult>;
    
    /**
     * Analyze multiple packages
     * @param packages Array of package requests containing name and version
     */
    public async analyze(packages: PackageRequest[]): Promise<MultiPackageAnalysisResult>;
    
    /**
     * Implementation of the analyze method
     */
    public async analyze(packageName: string | PackageRequest[], version?: string): Promise<AnalysisResult | MultiPackageAnalysisResult> {
        if (Array.isArray(packageName)) {
            return await this.analyzeMultiple(packageName);
        } else if (version) {
            return await this.analyzeSingle(packageName, version);
        } else {
            throw new Error('Invalid arguments: When analyzing a single package, both name and version are required');
        }
    }
}
